...
Open your browser and navigate to: `https://realtime-messaging-15.preview.emergentagent.com`
...
**Via API:**
```bash
curl -X GET "https://realtime-messaging-15.preview.emergentagent.com/api/send?number=9876543210&msg=Hello+World" \
...
async function sendMessage(number, message, apiKey) {
  try {
    const response = await axios.get(
      `https://realtime-messaging-15.preview.emergentagent.com/api/send`,
      {
...
def send_message(number, message, api_key):
    url = 'https://realtime-messaging-15.preview.emergentagent.com/api/send'
...
### cURL
```bash
curl -X GET "https://realtime-messaging-15.preview.emergentagent.com/api/send?number=9876543210&msg=Hello+from+cURL" \
  -H "api_key: YOUR_API_KEY"
```
